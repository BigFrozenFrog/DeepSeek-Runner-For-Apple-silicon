...
struct ModelLoadView: View {
    ...
    var body: some View {
        Form {
            ...
            Section("Percorsi") {
                HStack {
                    ...
                    Button { showModelPicker = true } label: { Label("Sfoglia…", systemImage: "folder") }
                        .buttonStyle(.borderless)
                }
                Text("Sotto sandbox il modello va scelto qui: l'accesso viene concesso e ricordato.")
                    .font(.caption).foregroundStyle(.secondary)
        ...
        .fileImporter(isPresented: $showModelPicker,
            ...
            if case .success(let urls) = result, let u = urls.first { store.chooseModel(u) }
        }
