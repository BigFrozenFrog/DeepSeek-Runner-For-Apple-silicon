...
extension GraphContext {
    ...
    /// In-place 128-wide Hadamard rotation + E2M1FN FP4 QAT of `nRows` rows (the
    /// indexer's emit quantization). kernel_dsv4_indexer_hadamard_fp4_f32 (head_dim=128).
    func indexerQatRowsEnc(_ x: GPUTensor, nRows: Int) throws {
        let headDim = 128
        var args = [UInt8](repeating: 0, count: 16)
        func u32(_ off: Int, _ v: UInt32) { withUnsafeBytes(of: v.littleEndian) { for k in 0..<4 { args[off+k] = $0[k] } } }
    ...
    func runCompressor(attnNorm: GPUTensor, w: LayerWeights, comp: CompressorState,
        ...
        return try runCompressorLane(attnNorm: attnNorm, compKv: compKv, compGate: compGate, ape: ape, normW: normW,
                                     comp: comp, rope: rope, pos: pos, rmsEps: rmsEps, nRot: nRot, emit: .fp8)
    }

    /// Generalized one-token compressor lane. The attention lane (headDim=512) emits via
    /// fp8; the indexer lane (headDim=128) emits via the Hadamard+FP4 QAT. Same recurrent
    /// store -> pool -> rmsNorm -> rope -> quant -> cache -> shift.
    func runCompressorLane(attnNorm: GPUTensor, compKv: GPUTensor, compGate: GPUTensor, ape: GPUTensor,
                           normW: GPUTensor, comp: CompressorState, rope: RopeParams, pos: Int,
                           rmsEps: Float, nRot: Int, emit emitMode: CompressorEmit) throws -> Int {
        let h = comp.headDim, ratio = comp.ratio, width = comp.width
        ...
        let isEmit = ((pos + 1) % ratio) == 0
        if !isEmit { return comp.count }
        ...
        try ropeTail(x: comp.rowScratch, nTok: 1, nHead: 1, headDim: h, nRot: min(nRot, h), nCtxOrig: rope.nCtxOrig,
        ...
        case .indexerQat: try indexerQatRowsEnc(comp.rowScratch, nRows: 1)
        }
...
/// Emit-time quantization for a compressed row: attention lane = E4M3 fp8;
/// indexer lane = 128-wide Hadamard + E2M1 FP4 QAT.
public enum CompressorEmit { case fp8, indexerQat }
