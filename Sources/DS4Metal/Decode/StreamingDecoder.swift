...
public final class StreamingDecoder {
    ...
    /// Fully no-copy path: BOTH dense and experts are no-copy mmap, driven by the
    /// single-cb decodeLayer (moeMatvecID reads the 6 selected experts by real id
    /// in-kernel). Vs fromGGUFExpertCachedMapped this drops the per-layer router-id
    /// readback + the second command buffer + the gather copy — fewer GPU syncs/token.
    /// Relies on Metal paging only the touched expert rows on demand. No memoization.
    public static func fromGGUFMappedAll(rt: MetalRuntime, model: GGUFModel, dims: DSV4Dims, rope: RopeParams,
                                         nLayers: Int, maxKeys: Int, rmsEps: Float = 1e-5, hcEps: Float = 1e-3) throws -> StreamingDecoder {
        let (embed, head) = try GGUFWeights.outputHeadMapped(rt, model)
        return try StreamingDecoder(rt: rt, dims: dims, rope: rope, nLayers: nLayers,
                                    layerProvider: { try GGUFWeights.layerMappedAll(rt, model, $0) },
                                    embedTable: embed, out: head, maxKeys: maxKeys, rmsEps: rmsEps, hcEps: hcEps,
                                    expertGather: nil)   // single-cb decodeLayer, real ids in-kernel
    }
