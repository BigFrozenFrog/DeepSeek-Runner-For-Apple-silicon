...
public struct DSV4Dims {
    ...
    public var swa: Int = 128                // sliding-window attention: raw attention covers only the
                                             // last `swa` tokens (DS4_N_SWA); older context comes from the
                                             // compressed rows. Without this, attention past 128 tokens
                                             // over-includes early raw keys the C excludes -> output collapse.
    public var sparseThreshold: Int = 1024   // DS4 default: above this many compressed rows the C selects a
                                             // subset; below, it attends all. We match below; above we apply
                                             // a RECENCY cap (the indexer's relevance top-k is the full impl).
    public var indexerTopK: Int = 512        // DS4_N_INDEXER_TOP_K: bound the compressed attention to this
                                             // many rows once past `sparseThreshold` (recency approximation).
...
/// NSA indexer weights (present only on ratio-4 compressed layers). Drives the
/// >4096-token sparse path: a second compressor lane (indexer head_dim=128, emit
/// via Hadamard+FP4 QAT) + per-compressed-row relevance scoring + top-512 select.
public struct IndexerWeights {
    public var compKv: GPUTensor, compGate: GPUTensor, compApe: GPUTensor, compNorm: GPUTensor  // indexer_compressor_*
    public var qB: GPUTensor      // indexer.attn_q_b  (qRank -> n_indexer_head*head_dim)
    public var proj: GPUTensor    // indexer.proj      (nEmbd -> n_indexer_head)
    public init(compKv: GPUTensor, compGate: GPUTensor, compApe: GPUTensor, compNorm: GPUTensor, qB: GPUTensor, proj: GPUTensor) {
        self.compKv = compKv; self.compGate = compGate; self.compApe = compApe; self.compNorm = compNorm
        self.qB = qB; self.proj = proj
    }
}
...
public struct LayerWeights {
    ...
    public var indexer: IndexerWeights?   // NSA indexer (ratio-4 layers; >4096-tok sparse path)
...
extension GraphContext {
    ...
    public func decodeRoute(curHc: GPUTensor, w: LayerWeights, s: DecodeScratch, d: DSV4Dims,
        ...
        // 4) attention over the SWA window of rawCache (last `swa` rows) + comp.cache[0..nComp].
        //    Older-than-window context is represented by the compressed rows; the C caps raw
        //    attention at DS4_N_SWA=128, so we must too or the distribution collapses past 128.
        let nRaw = min(nKeys, d.swa)
        let rawStart = nKeys - nRaw
        let rawWindow = rawStart == 0 ? rawCache : rawCache.rowView(row: rawStart, cols: d.headDim)
        // Compressed attention: match C (attend all rows) until past `sparseThreshold`,
        // then cap to the most recent `indexerTopK` rows (recency approximation of the
        // indexer's relevance top-k) so very long contexts stay bounded, not collapsing.
        let nCompAttend = nComp > d.sparseThreshold ? min(nComp, d.indexerTopK) : nComp
        let compStart = nComp - nCompAttend
        let compView: GPUTensor? = comp.map { compStart == 0 ? $0.cache : $0.cache.rowView(row: compStart, cols: d.headDim) }
        try flashAttnCore(q: s.q, kvF32: rawWindow, kvF16: s.kvF16, mask: s.mask, sinks: w.attnSinks,
                          pad: s.pad, tmp: s.tmp, heads: s.heads, nHead: d.nHead, nKeys: nRaw, hasSinks: true,
                          comp: compView, nComp: nCompAttend)
