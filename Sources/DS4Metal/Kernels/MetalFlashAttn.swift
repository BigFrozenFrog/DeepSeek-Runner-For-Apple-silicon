...
extension MetalRuntime {
    ...
    /// Float32 -> IEEE-754 binary16 bit pattern (round to nearest). Avoids the
    /// `Float16` type, which `import Metal` shadows with a C typedef in some build
    /// configs (the x86_64 Archive slice). Only used by the test helper below.
    static func halfBits(_ value: Float) -> UInt16 {
        let bits = value.bitPattern
        let sign = UInt16(truncatingIfNeeded: (bits >> 16) & 0x8000)
        let exp = Int((bits >> 23) & 0xFF)
        let mant = Int(bits & 0x007F_FFFF)
        if exp == 0xFF { return sign | 0x7C00 | (mant != 0 ? 0x0200 : 0) }   // Inf / NaN
        let e = exp - 127 + 15
        ...
        if e <= 0 {                                                          // subnormal / zero
            ...
            let shift = 14 - e
            var h = m >> shift
            if (m >> (shift - 1)) & 1 == 1 { h += 1 }
            return sign | UInt16(truncatingIfNeeded: h)
        }
        var h = (e << 10) | (mant >> 13)                                     // normal
        if (mant >> 12) & 1 == 1 { h += 1 }                                  // round
        return sign | UInt16(truncatingIfNeeded: h)
    }
    ...
    public func flashAttnDecode(q: [Float], kv: [Float], nHead: Int, nKeys: Int) throws -> [Float] {
        ...
        // K/V as F16 (same values the cpy kernel would produce). We convert with an
        // explicit float32->binary16 routine instead of the `Float16` type: `import
        // Metal` shadows the stdlib `Float16` with a C typedef (no `bitPattern`) in the
        // x86_64 slice of an Archive (universal) build, breaking compilation there.
        var kvF16 = [UInt16](repeating: 0, count: nKeys * headDim)
        for i in 0..<kvF16.count { kvF16[i] = Self.halfBits(kv[i]) }
