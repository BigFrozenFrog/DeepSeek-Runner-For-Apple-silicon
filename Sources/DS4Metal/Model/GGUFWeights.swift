...
public enum GGUFWeights {
    ...
    public static func layer(_ rt: MetalRuntime, _ model: GGUFModel, _ il: Int, loadExperts: Bool = true) throws -> LayerWeights {
        ...
        return LayerWeights(
            ...
            indexer: try indexerWeights(rt, model, p, mapped: false))
    }
    ...
    public static func layerMappedDense(_ rt: MetalRuntime, _ model: GGUFModel, _ il: Int) throws -> LayerWeights {
        ...
        return LayerWeights(
            ...
            compApe: try optT("attn_compressor_ape.weight"), compNorm: try optT("attn_compressor_norm.weight"),
            indexer: try indexerWeights(rt, model, p, mapped: true))
    }

    /// Fully no-copy layer: dense weights no-copy (like layerMappedDense) AND the
    /// routed experts as no-copy mmap views over the full expert tensors. Used with
    /// the single-command-buffer decodeLayer path (moeMatvecID reads the 6 selected
    /// rows by their real ids in-kernel) — no per-layer router-id readback, no gather
    /// copy, just one cb per layer. Requires model opened metalMapping:true.
    public static func layerMappedAll(_ rt: MetalRuntime, _ model: GGUFModel, _ il: Int) throws -> LayerWeights {
        var w = try layerMappedDense(rt, model, il)
        let p = "blk.\(il)."
        w.expGate = try mappedTensor(rt, model, p + "ffn_gate_exps.weight")
        w.expUp   = try mappedTensor(rt, model, p + "ffn_up_exps.weight")
        w.expDown = try mappedTensor(rt, model, p + "ffn_down_exps.weight")
    ...
    /// Load the NSA indexer weights for a layer if present (ratio-4 layers only).
    /// Big projections no-copy when `mapped`, the small APE/norm always copied.
    static func indexerWeights(_ rt: MetalRuntime, _ model: GGUFModel, _ p: String, mapped: Bool) throws -> IndexerWeights? {
        guard model.findTensor(p + "indexer.proj.weight") != nil else { return nil }
        func W(_ s: String) throws -> GPUTensor { mapped ? try mappedTensor(rt, model, p + s) : try tensor(rt, model, p + s) }
        func T(_ s: String) throws -> GPUTensor { try tensor(rt, model, p + s) }
        return IndexerWeights(
            compKv: try W("indexer_compressor_kv.weight"), compGate: try W("indexer_compressor_gate.weight"),
            compApe: try T("indexer_compressor_ape.weight"), compNorm: try T("indexer_compressor_norm.weight"),
            qB: try W("indexer.attn_q_b.weight"), proj: try W("indexer.proj.weight"))
    }
