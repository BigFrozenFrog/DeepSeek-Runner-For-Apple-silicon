...
enum OpenRouterError: LocalizedError {
    ...
    /// 402 path: the streaming client tried to retry with a shrunk
    /// `max_tokens` but the budget is still too tight. `lastAffordable`
    /// is the largest token count the key was billed for during the
    /// retry attempts — exposing it lets the user pick a setting in
    /// `GenerationSettingsTab` that they know will pass next time.
    case budgetExhausted(lastAffordable: Int)
    ...
    var errorDescription: String? {
        ...
        case .budgetExhausted(let lastAffordable):
            // Round down to the nearest 64 so the user can type the
            // number into the Settings stepper without it bouncing
            // off the next multiple. 64 is the same floor the retry
            // path uses.
            let safe = max(64, (lastAffordable / 64) * 64)
            return "OpenRouter rejected the request (HTTP 402): the "
                + "API key's per-request budget is exhausted even at "
                + "shrunk max_tokens. The last affordable size we saw "
                + "was \(lastAffordable) tokens — set max_tokens to "
                + "\(safe) or lower in Settings → Generation, switch "
                + "to a cheaper model, or top up the key at "
                + "openrouter.ai/keys."
        }
    }
        ...
        AsyncThrowingStream { continuation in
            let task = Task {
                do {
                    ...
                    // Track 402-retry state across the requestLoop. The
                    // first retry shrinks `max_tokens` to 85 % of the
                    // affordable hint; if the router lands on an even
                    // pricier upstream and 402s again, the second
                    // retry drops to 60 %. After two retries we give
                    // up — the key budget is genuinely below a usable
                    // window and the user needs to top up or pick a
                    // cheaper model.
                    var budgetRetries = 0
                    var lastAffordable: Int? = nil
                    let maxBudgetRetries = 2
                    let minRetryTokens = 64
                           ...
                           http.statusCode != 200 {
                            ...
                            if http.statusCode == 402,
                               ...
                               let affordable = Self.parseAffordableTokens(
                                from: body),
                               affordable >= minRetryTokens
                            {
                                // Progressive shrink: first retry at
                                // 85 % of the affordable hint, second
                                // at 60 %. Both floor at
                                // `minRetryTokens` so the request
                                // can't undershoot a usable reply.
                                lastAffordable = affordable
                                let margin: Double = budgetRetries == 0
                                    ? 0.85 : 0.60
                                let target = max(
                                    minRetryTokens,
                                    Int(Double(affordable) * margin))
                                bodyToSend["max_tokens"] = target
                                budgetRetries += 1
                                continue requestLoop
                            }
                            // Two retries spent (or affordable below
                            // floor / unparseable): surface the
                            // friendly error. When we have a hint,
                            // pass it through `OpenRouterError.http`
                            // so `errorDescription` can name it in
                            // Settings → Generation guidance.
                            if http.statusCode == 402, let hint = lastAffordable {
                                throw OpenRouterError.budgetExhausted(
                                    lastAffordable: hint)
                            }
