...
do {
    ...
    // Size the KV caches for the whole run (prompt + generation) so a long generation
    // doesn't overflow rawCache/comp_cache. ~512 margin for the prompt.
    let maxKeys = max(4096, maxNew + 512)
    let dec = try StreamingDecoder.fromGGUFExpertCachedMapped(rt: rt, model: model, dims: dims, rope: rope,
                                                              nLayers: DSV4Shape.nLayer, maxKeys: maxKeys)
    log("DS4Demo: no-copy mmap non-routed + gather 6 experts/token (maxKeys=\(maxKeys))…")
    let t0 = Date()
