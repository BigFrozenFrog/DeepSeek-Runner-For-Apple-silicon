...
///
/// Class (not struct) on purpose: the CLI does
/// `let w = SafeTensorsWriter()` then calls `w.add(...)` in a loop.
/// A struct with a mutating `add` would require `var`; making the
/// writer a reference type keeps the existing call sites compiling
/// without a churn pass through every `let`/`var` declaration.
///
